...
def P_T_overview_plot(model,var, timeframe, mode, diftype, inputplot, plotsave):
   ...
   # Set label and tick parameters for the colorbar
    ...
    if plotsave =='save': 
        if var=="Precipitation":
            plt.savefig(f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/04. Output files/Zoom/{timeframe}/{var}/1985.2014.{var}.{timeframe}.{mode}.{diftype}.{model}.zoom.png', bbox_inches='tight')
        else:
            plt.savefig(f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/04. Output files/Zoom/{timeframe}/{var}/1985.2014.{var}.{timeframe}.{mode}.abs.{model}.zoom.png', bbox_inches='tight')
    ...
    for var in ["Temperature", "Precipitation"]:
        for timeframe in ["monthly","seasonal","annual"]:
            for mode in ['dif']:#, 'std']:
                ...
                for dif in diftypes:
                    P_T_overview_plot(model, var, timeframe, mode, dif, 'off', 'nosave')
