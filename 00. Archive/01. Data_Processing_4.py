...
""" 
# fn  = f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/01. Input files/monthly/CESM2.DIF.000.clm.h0.1981.2014.ymonmean.total.nc'
# ds = nc.Dataset(fn, 'r')
# print(ds)
...
def individual_figures_zoom_subplot_test(var, model, member, total_members, zoom, timeframe, stat, datatype):
    ...
    # if datatype == 'DIF':
    fn  = f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/01. Input files/00. Archive/DIF0{member}/{model}.{datatype}.00{member}.clm.h0.1981.2014.{time_signature}{stat}.total.nc'

    # else:
    # fn  = f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/01. Input files/{timeframe}/{model}.{datatype}.00{member}.clm.h0.1981.2014.{time_signature}{stat}.nc'
    ...
    if stat=='std':
        total_members=1
    if datatype=='NOI' or datatype=='IRR':
        total_members=3
    ...
    for i in np.arange(0, total_members,1):
        ...
        for t, timestamp_name in enumerate(timestamps):
            #if datatype == 'DIF':
            fn_member  = f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/01. Input files/00. Archive/DIF0{i}/{model}.{datatype}.00{i}.clm.h0.1981.2014.{time_signature}{stat}.total.nc'
            #else: 
              #  fn_member  = f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/01. Input files/{timeframe}/{model}.{datatype}.00{i}.clm.h0.1981.2014.{time_signature}{stat}.nc'            
            
            ds_member=nc.Dataset(fn_member)
            ds_all = ds_member.variables[var][:, lat_indices, lon_indices]
            df = pd.DataFrame(ds_all[t,:,:])
            
            Q1 = df.quantile(0.25)
            Q3 = df.quantile(0.75)
                        
            min_scale[i,t]= np.min(Q1) #np.min(ds_all[t,:,:])
            max_scale[i,t]= np.max(Q3) #np.max(ds_all[t,:,:])
    ...
    if var=='RAIN_FROM_ATM_PERDAY' or var=='RAIN_FROM_ATM_PERDAY_REL':
        ...
        if stat =='std':
            print("Stat is std")
            colors = [(0, 'white'),(zero_scaled, 'xkcd:lightblue'),(1, 'xkcd:blue')]
            custom_cmap = LinearSegmentedColormap.from_list('custom_cmap', colors)  
        variable_type = 'RAIN_FROM_ATM_PERDAY'
        
    if var=='TSA' or var=='TSA_REL':
        #define legend scale based on local min& max values
        rounded_min = local_min#math.floor(local_min * 2) / 2
        rounded_max = local_max#math.ceil(local_max * 2) / 2
        ...
        if stat=='dif':
            ...
            custom_cmap = LinearSegmentedColormap.from_list('custom_cmap', colors)
        if stat =='std':
            colors = [(0, 'white'),(zero_scaled, 'xkcd:lightpurple'),(1, 'xkcd:purple')]
            custom_cmap = LinearSegmentedColormap.from_list('custom_cmap', colors)   
        variable_type = 'TSA'
    ...
    for time_idx, timestamp_name in enumerate(timestamps):
        ...
        if "REL" in var:
            min_value = var_zoom_time[min_value_index]*100
            max_value = var_zoom_time[max_value_index]*100
            ax.annotate(f'Min: {min_value:.2f}%',xy=(67,48), xytext=(67,48),fontsize=10, ha='left', va='top')
            ax.annotate(f'Max: {max_value:.2f}%', xy=(79,48), xytext=(79,48),fontsize=10, ha='left', va='top')
        else:
            min_value = var_zoom_time[min_value_index]
            max_value = var_zoom_time[max_value_index]
            ax.annotate(f'Min: {min_value:.2f}',xy=(69,48), xytext=(69,48),fontsize=10, ha='left', va='top')
            ax.annotate(f'Max: {max_value:.2f}', xy=(79,48), xytext=(79,48),fontsize=10, ha='left', va='top')
       
        ax.plot(68,47.8, marker='o', color='blue', markersize=10)
        ax.plot(78,47.8, marker='o', color='red', markersize=10)
       ...
       class ScalarFormatterForceFormat(ScalarFormatter):
           def __init__(self, useOffset=True, useMathText=True):
               ...
               def percent_formatter(x, pos):
                   x *= 100  # Multiply by 100 to represent as percentage
                   return f"{x:.0f}%"  # Format as percentage with two significant figures
               
               # Create a FuncFormatter object with the formatting function
               self.formatter = percent_formatter
    ...
    #save figure
    plt.savefig(f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/04. Output files/{model}/{variable_type}/00{member}/1981.2014.{timeframe}.{time_signature}.{datatype}.00{member}.{change_mode}.{stat}.{zoomed}.png', bbox_inches='tight')
    plt.show()
...
variable_types = ['RAIN_FROM_ATM_PERDAY','TSA']#, 'RAIN_FROM_ATM_PERDAY_REL','TSA']#, 'RAIN_FROM_ATM_PERDAY','TSA']#'RAIN_FROM_ATM_REL']#
timeframes = ['annual']#, 'seasonal', 'monthly']#, 'daily']
members =[0]#,1,2,3]#,1,2,3]
zoom_modes =['on']#, 'off']
datatypes = ["DIF"]#"NOI", "IRR"]#'DIF'
            ...
            for z, zoom in enumerate(zoom_modes):
                for d, datatype in enumerate(datatypes):
                    ...
                    for s,stat in enumerate(stats):
                        ...
                        figures = individual_figures_zoom_subplot_test(var_types,'CESM2', member, 1, zoom, timeframe, stat,datatype)
...
# figures = individual_figures_zoom_subplot_test('RAIN_FROM_ATM_PERDAY','CESM2','1', 4, 'on', 'monthly','mean', 'DIF')
...
def individual_figures_zoom_subplot(var, model,member,total_members, zoom, timeframe, stat):
    ...
    #load data from the indicated path (downloaded after processing in VSC)
    fn  = f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/01. Input files/{timeframe}/{model}.DIF.00{member}.clm.h0.1981.2014.{time_signature}{stat}.total.nc'
    ds = nc.Dataset(fn, 'r')    
    ...
    for i in np.arange(0, total_members,1):
        print(i)
        for t, timestamp_name in enumerate(timestamps):
            fn_member = f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/01. Input files/{timeframe}/{model}.DIF.00{i}.clm.h0.1981.2014.{time_signature}{stat}.total.nc'
            ds_member=nc.Dataset(fn_member)
            ds_all = ds_member.variables[var][:, lat_indices, lon_indices]
            ds_member=nc.Dataset(fn_member)
            ds_all = ds_member.variables[var][:, lat_indices, lon_indices]
    ...
    #save figure
    plt.savefig(f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/04. Output files/{model}/{variable_type}/00{member}/1981.2014.{timeframe}.{time_signature}dif.00{member}.{change_mode}.{stat}.{zoomed}.png', bbox_inches='tight')
    # plt.show()
