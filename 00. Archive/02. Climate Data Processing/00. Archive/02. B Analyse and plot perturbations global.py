...
def P_T_overview_plot(model,var, timeframe, mode, diftype, inputplot, plotsave):
    ...
    if model== "IPSL-CM6":
        ifile_IRR = ifile_IRR.rename_vars({'time_counter': 'time'})
        ifile_NOI = ifile_NOI.rename_vars({'time_counter': 'time'})
    print(ifile_IRR.variables)
    ...
    # baseline = baseline.where((baseline.lon >= 0) & (baseline.lon <= 179) & (baseline.lat >= 0) & (baseline.lat <= 179), drop=True) 
    baseline["time"] = baseline.time.astype("datetime64[ns]")
    baseline = baseline.groupby(time_averaging).mean()
    
    # irrigation = irrigation.where((irrigation.lon >= -179) & (irrigation.lon <= 179) & (irrigation.lat >= 0) & (irrigation.lat <= 179), drop=True) 
    ...
    # diff = diff.where((diff.lon >= -179) & (diff.lon <= 179) & (diff.lat >= 0) & (diff.lat <= 179), drop=True)
   ...
   # Set label and tick parameters for the colorbar
    ...
    if timeframe=='monthly':
        ...
        fig.text(0.5, 0.97, model , ha='center', fontsize=20)

        # ax.annotate(f'Min: {min_value:.1f}',xy=(-20,-75), xytext=(-20,-75),fontsize=15, ha='left', va='top')
        # ax.annotate(f'Max: {max_value:.1f}', xy=(85,-75), xytext=(85,-75),fontsize=15, ha='left', va='top')
        # ax.plot(-26,-78, marker='o', color='blue', markersize=5)
        # ax.plot(79,-78, marker='o', color='red', markersize=5)
        
    if timeframe=='seasonal':
        fig.text(0.5, 0.03, 'Longitude', ha='center', fontsize=15)
        fig.text(0.03, 0.5, 'Latitude', va='center', rotation='vertical', fontsize=15)
        fig.text(0.5, 0.93, model , ha='center', fontsize=20)
    if timeframe=='annual':
        fig.text(0.5, 0.1, 'Longitude', ha='center', fontsize=15)
        fig.text(0.03, 0.5, 'Latitude', va='center', rotation='vertical', fontsize=15)
        fig.text(0.5, 0.85, model , ha='center', fontsize=20)
    ...
    if plotsave =='save': 
        if var=="Precipitation":
            plt.savefig(f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/04. Output files/Global/{model}/1981.2014.{var}.{timeframe}.{mode}.{diftype}.global.png', bbox_inches='tight')
        else:
            plt.savefig(f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/04. Output files/Global/{model}/1981.2014.{var}.{timeframe}.{mode}.abs.global.png', bbox_inches='tight')
...
# for model in ["E3SM","CESM2", "CNRM", "IPSL-CM6"]: 
...
P_T_overview_plot("IPSL-CM6", "Temperature", "annual", "dif", "rel", 'off', 'nosave')
