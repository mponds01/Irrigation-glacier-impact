...
ifile_IRR="CESM2.IRR.000.clm.h0.1981_2014_selparam_monthly_total.nc"
ifile_NOI="CESM2.NOI.000.clm.h0.1981_2014_selparam_monthly_total.nc"
...
# baseline = xr.open_dataset(folder+ifile_NOI).RAIN_FROM_ATM*86400*30
# baseline = baseline.where((baseline.lon >= 60) & (baseline.lon <= 100) & (baseline.lat >= 20) & (baseline.lat <= 50), drop=True) 
# baseline["time"] = baseline.time.astype("datetime64[ns]")
...
# plt.figure()
# baseline_R.where(baseline_R['time.year'] > 1970, drop=True).sel(lat = 40, lon = 80, method = "nearest").plot(label="R", color="peachpuff")
# baseline_S.where(baseline_S['time.year'] > 1970, drop=True).sel(lat = 40, lon = 80, method = "nearest").plot(label="S", color="lightblue")
