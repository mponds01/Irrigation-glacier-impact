...
for gdir in gdirs_3r_a5:
    ...
    with open(gdir_path, 'wb') as f:
        pickle.dump(gdir, f)
# %% Cell 3c: Load gdirs_3r_a5 from pkl
# check wd path before running
wd_path_pkls = f'{wd_path}/pkls/'
...
# %% Cell 4: Create a dataset that contains gdir Area, volume, rgi date and rgi ID
...
print(df_3r.head)
# %% Cell 4b: Add the subregion ids to the masterdataset

df_3r_a5 = pd.read_csv(f"{wd_path}/masters/master_gdirs_r3_rgi_date_A_V.csv")
...
merged_df = pd.merge(df_3r_a5, subregions, on='rgi_subregion')

new_order = ['rgi_id', 'rgi_region', 'rgi_subregion', 'full_name', 'cenlon', 'cenlat', 'rgi_date',
             'rgi_area_km2', 'rgi_volume_km3']

merged_df = merged_df[new_order]
merged_df.to_csv(f"{wd_path}/masters/master_gdirs_r3_rgi_date_A_V_RGIreg.csv")


# %% Cell 5: Plot total cumulative volume vs area (descending)

df_3r = pd.read_csv(
    f"{wd_path}/masters/master_gdirs_r3_rgi_date_A_V_RGIreg.csv")
...
# Show plot
save_folder = f"{fig_path}/00. Glacier Subset/"
os.makedirs(fig_folder, exist_ok=True)
plt.savefig(f"{save_folder}/Glacier_subset_area_volume_overview.png")
plt.show()

# %% Cell 6: Process the Irr climate perturbations for all the gdirs and compile
...
# %% Cell 7: Perturb the climate historical with the processed Irr-perturbations, output is gcm file
cfg.PARAMS['use_multiprocessing'] = True
count = 0
# gdir = gdirs_3r_a5[0]
for gdir in gdirs_3r_a5:
...
# %% Cell 8: Run Mass Balance model for the glaciers (V = A*B)
...
# %% Cell 9: Update the master dataset with B, A, RGI ID and location for all the different sample ids (541x15=8115) and the region name
...
# %% Cell 10 plot mass balance data in histograms and gaussians (subplots and one plot for all member options, boxplot and gaussian only option)
...
plt.tight_layout()
save_folder = os.path.join(fig_folder, "03. Mass Balance", "Histogram")
os.makedirs(save_folder, exist_ok=True)
plt.savefig(f"{save_folder}/Gaussian_distribution_total_region_by_member.png")

plt.show()

# %% Cell 11: plot mass balance data in histograms and gaussians - by subregion (subplots and one plot for all member options, boxplot and gaussian only option)
...
if subplots == False:
    fig.legend(handles=region_legend_patches, loc='center left',
               bbox_to_anchor=(0.13, 0.6), ncols=1)
else:
    fig.legend(handles=region_legend_patches, loc="center",
               bbox_to_anchor=(1.05, 0.76), ncols=1)

ax.set_xlim(-1500, 1000)
plt.tight_layout()

save_folder = os.path.join(fig_path, "03. Mass Balance", "Histogram")
os.makedirs(save_folder, exist_ok=True)
if subplots == False:
    plt.savefig(
        f"{save_folder}/Gaussian_distribution_total_region_by_region_1plot.png")
if subplots == True:
    plt.savefig(
        f"{save_folder}/Gaussian_distribution_total_region_by_region_subplots.png")

plt.show()


# %% Cell 12: Plot the specific mass balances using boxplots, different subsets, weighted by region - print median
# Initialize plot
...
master = pd.read_csv(f"{wd_path}masters/master_gdirs_r3_a5_rgi_date_A_V_B.csv")
...
# use this dataset to show aggregated differences with size in volume
...
# use this dataset to use aggregated Irr B and aggregated by rgi area
aggregated_ds = master_ds_avg.groupby(['grid_lon', 'grid_lat'], as_index=False).agg(
    B_delta_aggregated=('B_delta', 'sum'),
    B_irr_aggregated=('B_irr', 'sum'),
    B_noirr_aggregated=('B_noirr', 'sum'),
    A_aggregated=('rgi_area_km2', 'sum'),
    V_aggregated=('rgi_volume_km3', 'sum')
)
...
scatter = ax.scatter(gdf.geometry.x, gdf.geometry.y,
                     s=np.log(gdf['A_aggregated'])*10, c=gdf['B_noirr_aggregated'], cmap=custom_cmap, norm=norm, edgecolor='k', alpha=1)
...
custom_sizes = [50, 500, 5000]  # Example sizes for the legend
