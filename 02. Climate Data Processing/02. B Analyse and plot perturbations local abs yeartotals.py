...
def P_T_overview_plot(model,var, timeframe, mode, diftype, inputplot, plotsave):
    ...
    ifile_IRR=f"{model}.IRR.000.1981_2014_selparam_monthly_{mode_suff}.nc"
    ifile_NOI=f"{model}.NOI.000.1981_2014_selparam_monthly_{mode_suff}.nc"
    ...
    if model== "IPSL-CM6":
        ...
        # print(ifile_IRR.keys)
    # print(ifile_IRR.data_vars)
    ...
    if var =="Precipitation":
        ...
        print(irrigation.sel(lon=80, lat=35, method='nearest'))
    ...
    if timeframe=='annual':
        mean_baseline=round(baseline.mean(dim=['lat','lon']).item(),2)
        mean_irrigation=round(irrigation.mean(dim=['lat','lon']).item(),2)
        mean_diff=round(diff.mean(dim=['lat','lon']).item(),2)
    ...
    shapefile_path = '/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/01. Input files/Shapefile/Karakoram/Liu_2022/Pan_Tibetan_shape.shp'
    ...
    for time_idx, timestamp_name in enumerate(timestamps):
        ...
        if timeframe=='annual':
            ...
            irr_sel = irrigation.isel({irrigation.dims[0]: time_idx})
      
        #plot the data incl the outline of the karakoram shapefile, setting the colors, but excluding the shapefile
        #print(vmin, vmax)
        im = irr_sel.plot(ax=ax, vmin=vmin, vmax=vmax, extend='both', transform=ccrs.PlateCarree(), cmap=custom_cmap, add_colorbar=False)
        ...
        diff_sel_min = irr_sel.fillna(baseline.max())
        diff_sel_max = irr_sel.fillna(baseline.min())
        ...
        min_lon, min_lat = irr_sel.lon.values[min_value_index[1]], irr_sel.lat.values[min_value_index[0]]
        max_lon, max_lat = irr_sel.lon.values[max_value_index[1]], irr_sel.lat.values[max_value_index[0]]
        ...
        min_value = irr_sel[min_value_index]
        max_value = irr_sel[max_value_index]
        ax.annotate(f'Min: {min_value:.1f}',xy=(62,51), xytext=(62,51),fontsize=15, ha='left', va='top')
        ax.annotate(f'Max: {max_value:.1f}', xy=(78,51), xytext=(78,51),fontsize=15, ha='left', va='top')
      
        ax.plot(61,50.5, marker='o', color='blue', markersize=5)
        ax.plot(77,50.5, marker='o', color='red', markersize=5)
    ...
    if plotsave =='save': 
         if var=="Precipitation":
             plt.savefig(f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/04. Output files/Test/New_totals/{model}.1981.2014.{var}.{timeframe}.{mode}.{diftype}.zoom.png', bbox_inches='tight')
         else:
             plt.savefig(f'/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/04. Output files/Test/New_totals/{model}.1981.2014.{var}.{timeframe}.{mode}.abs.zoom.png', bbox_inches='tight')
    ...
    return mean_irrigation, mean_baseline, mean_diff
     
    
for var in ["Precipitation","Temperature"]:
    df = pd.DataFrame(columns=['Model', 'Mean_IRR','Mean_NOI', 'Mean_DIF'])
    data_to_concat=[]
    
    for i, model in enumerate (["CESM2", "CNRM", "E3SM", "IPSL-CM6"]): #
        for timeframe in ["annual"]:#, "seasonal", "monthly"]:
            for mode in ['dif']:#, 'std']:
                diftypes=['abs']                
                for dif in diftypes:
                    # P_T_overview_plot(model, var, timeframe, mode, dif, 'off', 'save')
                    Mean_IRR, Mean_NOI, Mean_DIF= P_T_overview_plot(model, var, timeframe, mode, dif, 'off', 'nosave')
                    new_data = {'Model': model, 'Mean_IRR': Mean_IRR, 'Mean_NOI': Mean_NOI, 'Mean_DIF': Mean_DIF}
                    data_to_concat.append(pd.DataFrame([new_data]))
    df = pd.concat(data_to_concat, ignore_index=True)
    df.to_excel(f"/Users/magaliponds/OneDrive - Vrije Universiteit Brussel/1. VUB/02. Coding/01. IRRMIP/02. Data/04. Output files/Test/New_totals/{var}_abs_mean_values_output.xlsx", index=False)
    
                    # Append the new row to the existing DataFrame
                    # df = df.append(new_data, ignore_index=True)
    print(df)
                             
     # P_T_overview_plot("E3SM", "Temperature", "monthly", "dif", "rel", 'off', 'nosave')
