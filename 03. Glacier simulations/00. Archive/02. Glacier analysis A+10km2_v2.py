...
save_dir = '/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/02. Data/06. OGGM Working Directories/03. Saved gdirs (subset)' 
...
plt.figure(figsize=(10,5))
plt.hist(stats.melt_f.values, bins=np.linspace(stats['melt_f'].min(), stats['melt_f'].max(), int(len(stats['melt_f'])/7)), align='left', rwidth=0.8, color='red')
plt.ylabel('# of glaciers')
plt.xlabel('melt_f (mm w.e. d⁻¹ °C⁻¹)')

plt.figure(figsize=(10,5))
plt.hist(stats.prcp_fac.values, bins=np.linspace(stats['prcp_fac'].min(), stats['prcp_fac'].max(), int(len(stats['prcp_fac'])/7)), align='left', rwidth=0.8, color='blue')
plt.ylabel('# of glaciers')
plt.xlabel('prcp_fac (-)')

plt.figure(figsize=(10,5))
plt.hist(stats.temp_bias.values, bins=np.linspace(stats['temp_bias'].min(), stats['temp_bias'].max(), int(len(stats['temp_bias'])/7)), align='left', rwidth=0.8, color='orange')
plt.ylabel('# of glaciers')
plt.xlabel('temp_bias (°C)')

plt.figure(figsize=(10,5))
error_perc = stats.dmdtda_dynamic_calibration_given_error/stats.dmdtda_mismatch_dynamic_calibration_reference
plt.hist(error_perc, bins=np.linspace(error_perc.min(), error_perc.max(), int(len(error_perc)/7)), align='left', rwidth=0.8, color='pink')
plt.xlim((-2,2))
