...
#%% OUTDATED -  Create map plot #dense call out plots
...
shp.plot(ax=ax, edgecolor='k', linewidth=0, facecolor='lightgrey')
...
# Create the BoundaryNorm with the defined boundaries
cax = fig.add_axes([0.6, -0.14, 0.37, 0.02])  # [left, bottom, width, height]
...
cbar.ax.tick_params(labelsize=14)  # Create the colorbar
cbar.set_label('$B_{Irr}$ (m w.e. yr$^{-1}$)',
               fontsize=16)  # Label for the colorbar
...
legend_handles = [
    plt.scatter([], [], s=np.sqrt(size)*3, edgecolor='k', facecolor='none')
...
cax_legend = fig.add_axes([w_leg, h_leg, 0.4, 0.05])  # [left, bottom, width, height]
...
x_min, x_max = start_x, start_x + 5 * (w + w_space)
y_min, y_max = start_y, start_y - (h + h_space) * 4 - y_buffer
...
for idx, pos in enumerate(grid_positions):
    if pos:
        ...
        region_id = layout[idx // 5][idx % 5] #index columns and rows, // is rounded by full nrs
        print(region_id)
        ...
        if vol_display =="absolute":
            ...
            ax_callout.plot(climate_run_output["time"].values, mean_values,
                            color="blue", linestyle='solid', lw=2, label=f"{sum(members_averages)}-member average")
...
fig_legend = fig.add_axes([-0.07, -0.07*7.5, w*3.59, h*3.59])  # make twice as large as the callout plots
...
fig_legend.legend(loc='center left', bbox_to_anchor=(1.07, 0.25), ncol=1, fontsize=14,
                  frameon=False, labelspacing=2)

# Set labels for axes
fig_legend.set_xlabel('Time', fontsize=14)
fig_legend.set_ylabel('Volume [km$^3$]', fontsize=14)
...
for v, var in enumerate(variables):
    print(var)
    # create a new plot for both area and volume
    fig, ax = plt.subplots(figsize=(13,10))  # create a new figure
...
# %% Cell 11c: Plot comitted data - % axis
...
variable_axes = ["Volume compared to 1985 All Forcings scenario [%]",
                 "Area compared to 1985 All Forcings scenario [%]"]
# variable_axes = ["Volume compared to 1985 Irr-scenario [m3]",
#                  "Area compared to 1985 Irr-scenario [m3]"]
use_multiprocessing = False
...
for v, var in enumerate(variables):
    ...
    for f, filepath in enumerate([f"climate_run_output_baseline_W5E5.000.nc", f"climate_run_output_baseline_W5E5.000_comitted_random.nc"]):
        ...
        if f == 0:
            ...
            legendtext=f"AllForcings {legend_id}"
        else:
            legendtext=None
        ax.plot(baseline["time"], (baseline[var].sum(dim="rgi_id") * factors[v])/resp_value*100,
                label=legendtext, color=colors[f"irr{color_id}"][0], linewidth=2, zorder=15, linestyle=linestyles[f])

        mean_values_irr = (baseline[var].sum(
            dim="rgi_id") * factors[v]).values
        
        if f==1:
            print(var, "baseline", baseline["time"].values[-1],(baseline[var].sum(dim="rgi_id").values[-1] * factors[v])/resp_value*100)
            ...
            if r>=1:
                for m, model in enumerate(models_shortlist):
                    for i in range(members_averages[m]):
                        ...
                        # print(f"time range CRO start {run_type}:", climate_run_output_noirr.volume[0].values)
                        # print(f"time range CRO end {run_type}:", climate_run_output_noirr.volume[-1].values)
                ...
                # mean_values_noirr = np.mean(stacked_member_data, axis=0).flatten()
                if f==0:
                    labeltext = f"{run_label} (14-member avg) {legend_id}"
                    # labeltext = f"{run_label} ({sum(members_averages)}-member avg) {legend_id}"
                else:
                    labeltext = None
                ax.plot(climate_run_output_noirr["time"].values, mean_values_noirr,
                        color=colors[f"{run_type}{color_id}"][f], linestyle=linestyles[f], lw=2, label=labeltext, zorder=5)
                if f==1:
                    print(var, r, climate_run_output_noirr["time"].values[-1],mean_values_noirr[-1])
    
                # calculate and plot volume/area 10-member min and max for ribbon
                min_values_noirr = np.min(stacked_member_data, axis=0).flatten()
                max_values_noirr = np.max(stacked_member_data, axis=0).flatten()
                if f==0:
                    labeltext = f"{run_label} (14-member range) {legend_id}"
                    # labeltext = f"{run_label} ({sum(members_averages)}-member range) {legend_id}"
                else:
                    labeltext=None
                ax.fill_between(climate_run_output_noirr["time"].values, min_values_noirr, max_values_noirr,
                                color=colors[f"{run_type}{color_id}"][f], alpha=0.3, label=labeltext, zorder=16)
...
# %% Cell 11d: Plot comitted data by region
