...
colors = {
    ...
    "irr_fut": [ "#1B3968","#D6102A",],  # SSP370: deep red, SSP126: deep blue
    "noi_fut": ["#A4B9E1", "#F8AEB8"],  # SSP370: pale rose, SSP126: light periwinkle
}
            ...
            for s, ssp in enumerate(ssps):
                for e, ex in enumerate(exp):
                    if member >= 1: #skip 0
                        ...
                        ax = axes[g]
                        dir_path = os.path.join(root_dir, "Magali 3", "per_glacier",gdir.rgi_id[:8], gdir.rgi_id[:11], gdir.rgi_id)
                        gdir_file = os.path.join(dir_path, f"model_diagnostics{out_id}.nc")
                        
                        with xr.open_dataset(gdir_file) as ds:
                            ds.load()
                            ax.plot(ds.time, ds.volume_m3*factor, color=color)
                            gdir_avg_past.append(ds.volume_m3*factor)
                            
                        gdir_file=f"{dir_path}/model_diagnostics{out_id}.nc" 
                        with xr.open_dataset(gdir_file) as ds:
                            ds.load()
...
#%% create master for creating subplots - outdated
exp = ["IRR", "NOI"]
...
#%% Process hydro data to monthly timeseries and create output plot
...
for e,ex in enumerate(["irr", "noi"]):
        ...
        for member in range(members[m]):
            if member>=1:
                ...
                with xr.open_dataset(hydro_data_past) as ds:
                    ...
                    monthly_runoff = ds['melt_on_glacier_monthly'] 
                    monthly_runoff = monthly_runoff.sum(dim='rgi_id')
                    ...
                    df_monthly = monthly_runoff.to_pandas()
                    df_monthly = df_monthly.stack().reset_index()
                    df_monthly.columns = ['year', 'month', 'runoff']
                    ...
                    for i, yr in enumerate([1985]):
                        if ex=='irr':
                            ...
                            ls=':'
                            monthly_runoff.sel(time=yr).plot(ax=ax, color=colors[f'{ex}'][0],  ls=ls, lw=1, zorder=10)#label=f'Year {yr}',
    ...
    for i,yr in enumerate([1985]):
        if ex=='irr':
            ...
            ls='--'
        df_subset = df_monthly_all_past[(df_monthly_all_past['experiment'] == ex) & (df_monthly_all_past['year'] == yr)]
        df_stats = df_subset.groupby('month')['runoff'].agg(['min', 'max', 'mean']).reset_index()
        # df_avg_monthly = df_subset.groupby('month')['runoff'].mean()#.reset_index()
        ax.plot(df_stats['month'], df_stats['mean'],color=colors[f'{ex}'][0], ls=ls, lw=3, zorder=100)#label=f'Year {yr}',
        ax.fill_between(df_stats['month'], df_stats['min'], df_stats['max'], color=colors[f'{ex}'][1],alpha=0.3, zorder=50 )#, label='Range (min–max)')
    ...
    for s, ssp in enumerate(["126", "370"]):
        for member in range(4): #4
            if member>=1:
                ...
                with xr.open_dataset(hydro_data_path) as ds:
                    ...
                    ## Create line plot for total annual runoff
                    # df_runoff.sum(axis=1).plot(ax=ax) #sum for all Rgi_ids and for total runoff
                    # df_runoff.sum(axis=1).plot(ax=ax);
                    
                    ## Create relative contributio per runoff component
                    # df_runoff.plot.area(ax=ax, color=sns.color_palette("rocket")); plt.xlabel('Years'); plt.ylabel('Runoff (Mt)'); plt.title("HMA total runoff");
                    # plt.ylabel('Mt'); plt.xlabel('Years'); plt.title(f'Total annual runoff for HMA A>5km2');
                    
                    ## Monthlky runoff 
                    monthly_runoff = ds['melt_on_glacier_monthly'] 
                    # (ds['melt_off_glacier_monthly'] + ds['melt_on_glacier_monthly'] +
                    ...
                    for i, yr in enumerate([2073]):
                        ...
                        if ex=='irr':
                            ...
                            ls=':'
                        monthly_runoff.sel(time=yr).plot(ax=ax, color=colors[f'{ex}{colorlabel}'][s],  ls=ls, lw=0.5, zorder=10)#label=f'Year {yr}',
        ...
        for i,yr in enumerate([2073]):
            if ex=='irr':
                ...
                ls='--'
            df_subset = df_monthly_all[(df_monthly_all['ssp'] == f'SSP{ssp}') & (df_monthly_all['experiment'] == ex) & (df_monthly_all['year'] == yr)]
            df_stats = df_subset.groupby('month')['runoff'].agg(['min', 'max', 'mean']).reset_index()
            # df_avg_monthly = df_subset.groupby('month')['runoff'].mean()#.reset_index()
            ax.plot(df_stats['month'], df_stats['mean'],color=colors[f'{ex}_fut'][s], ls=ls, lw=3, zorder=100)#label=f'Year {yr}',
            ax.fill_between(df_stats['month'], df_stats['min'], df_stats['max'], color=colors[f'{ex}_fut'][s],alpha=0.3, zorder=50 )#, label='Range (min–max)')
...
base_ds = pd.read_csv(f"{wd_path}/masters/master_gdirs_r3_a5_rgi_date_A_V.csv", usecols=['rgi_id','rgi_subregion']).set_index('rgi_id').to_xarray()
    ...
    for m, model in enumerate(models):
        for member in range(members[m]):
            if member >= 1:
                ...
                try:
                    with xr.open_dataset(hydro_data_past) as ds:
                        ds_runoff = (ds['melt_on_glacier_monthly'] + ds['melt_off_glacier'] + ds['liq_prcp_on_glacier'] + + ds['liq_prcp_off_glacier'])* 1e-9
                        runoff = xr.merge([ds_runoff.to_dataset(name='runoff'), base_ds]).set_coords('rgi_subregion')
                        runoff_subregions = runoff.groupby('rgi_subregion').sum(dim='rgi_id')

                        df = runoff_subregions.to_dataframe().reset_index()
                        df_clean = df[['time', 'month_2d', 'runoff', 'rgi_subregion']].copy()
                        df_clean = df_clean.rename(columns={
                            'time': 'year',
                            'month_2d': 'month'
                        })
                        df_clean['experiment'] = ex
                        df_clean['ssp'] = "W5E5"
                        df_clean['member'] = member
                        df_monthly_all.append(df_clean)
...
#%% Make hydro plot on seasonality average over time period per subregion
...
df_avg_subregions = (df_avg_monthly.groupby(['year', 'experiment', 'ssp', 'rgi_subregion'])['runoff'].sum().reset_index()) #sum for all subregions per year
...
for region in df_avg_subregions['rgi_subregion'].unique():
    region_data = df_avg_subregions[df_avg_subregions['rgi_subregion'] == region]
    ...
    for (exp, ssp), group in region_data.groupby(['experiment', 'ssp']):
        ...
        print(region_data[region_data['year']==1985][region_data['experiment']=='irr'])
        print(group)
        pct = ((group['runoff']-base_year)/base_year)*100
        ax[c].plot(group['year'],group['runoff'],#pct,#
...
for (exp, ssp), group in df_avg_all.groupby(['experiment', 'ssp']):
    print(group)
    if ssp == 'W5E5':
        add = ""
        s = 0  # default index
    ...
    ls = '--' if exp == "noi" else '-'
    lw = 1 if exp == "noi" else 2
    pct_2 = ((group['runoff']-base_year_all)/base_year_all)*100

    
    ax[0].plot(group['year'], group['runoff'],#pct_2,#
             label=f"{exp.upper()} {ssp}",
             color=colors[f'{exp}{add}'][s],
             linestyle=ls, linewidth=lw)
...
#%% Plot average seasonality in past and future
...
if boxplot == 'on':
    ...
    palette = {
        'irr_W5E5': colors['irr'][0],
        'noi_W5E5': colors['noi'][0],
