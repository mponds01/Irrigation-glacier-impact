...
opath_df_annual = os.path.join(wd_path,'masters','hydro_output_annual_subregions_per_rgi_id_preselected_stacked.csv')
df_combined.to_csv(opath_df_annual)


#%%  13d Process and calculate the peak water time per glacier
...
plot_df = merged_sum.dropna(subset=['year_of_max_runoff'])
...
for ssp in ['126','370']:
    for exp in ['irr', 'noi']:
        ...
        area_scaling = 1e6  # adjust to your liking
        ...
        scatter = ax.scatter(
            ...
            c=plot_df_sel['year_of_max_runoff'], cmap='viridis',
            s=marker_sizes, alpha=0.7, edgecolor='k',
            transform=ccrs.PlateCarree()
        )
        
        # Colorbar
        cbar = plt.colorbar(scatter, ax=ax, orientation='vertical', label='Peak Water Year')
        
        # Title and labels
        ax.set_title(f'Peak Water Year {ssp} x {exp}')
