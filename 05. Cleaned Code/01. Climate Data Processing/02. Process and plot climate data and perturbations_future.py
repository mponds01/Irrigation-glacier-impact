...
"""
This script processes the IRRMIP input data to perturbations (∆Irr-NoIrr) and plots them:
    1. Processes the IRRMIP data to monthly, annual or seasonal perturbations in temperature and precipitation - Future (2015-2074)
    2. Function for plotting future perturbations
    3. Loop thought the processing function for all model member combinations in IRRMIP (future)
    4. Loop through the plotting function for all model member combinations in IRRMIP (future)
"""
...
import pandas as pd
diff_folder_in = f"/Users/magaliponds/OneDrive - Vrije Universiteit Brussel/1. VUB/02. Research/01. IRRMIP/03. Data/01. Input files/01. Climate data/CESM2/2015/"
ifile_diff = f"{diff_folder_in}/CESM2.SSP126.IRR.01.2015_2074.selparam.monthly.total.nc"
# ifile_diff = f"{diff_folder_in}/CESM2.126_IRR.001.2015_selparam_monthly.nc"
ds = xr.open_dataset(ifile_diff)

# %% Cell 1: Process perturbations for IRR-NOI comparison

def process_P_T_perturbations(model, member, var, timeframe, mode, diftype, y0, ye, scenario):
    ...
    folder_in = f"/Users/magaliponds/OneDrive - Vrije Universiteit Brussel/1. VUB/02. Research/01. IRRMIP/03. Data/01. Input files/01. Climate data/{model}/{y0}/"
    ifile_IRR = f"{model}.SSP{scenario}.IRR.00{member}.{y0}_{ye}_selparam_monthly_{mode_suff}.nc"
    ifile_NOI = f"{model}.SSP{scenario}.NOI.00{member}.{y0}_{ye}_selparam_monthly_{mode_suff}.nc"
    ...
    base_folder_out = f"/Users/magaliponds/OneDrive - Vrije Universiteit Brussel/1. VUB/02. Research/01. IRRMIP/03. Data/03. Output files/01. Climate data/01. Processed input data/{var}/{timeframe}/{model}/{member}/SSP{scenario}"
    diff_folder_out = f"/Users/magaliponds/OneDrive - Vrije Universiteit Brussel/1. VUB/02. Research/01. IRRMIP/03. Data/03. Output files/01. Climate data/02. Perturbations/{var}/{timeframe}/{model}/{member}/SSP{scenario}"
    os.makedirs(diff_folder_out, exist_ok=True)
    os.makedirs(base_folder_out, exist_ok=True)

    ofile_diff = f"{diff_folder_out}/{model}.{var_suffix}.SSP{scenario}.DIF.00{member}.{y0}_{ye}_{timeframe}_{diftype}.nc"
    ofile_irr = f"{base_folder_out}/{model}.{var_suffix}.SSP{scenario}.IRR.00{member}.{y0}_{ye}_{timeframe}_{diftype}.nc"
    ofile_noi = f"{base_folder_out}/{model}.{var_suffix}.SSP{scenario}.NOI.00{member}.{y0}_{ye}_{timeframe}_{diftype}.nc"
    print(ofile_diff)
...
def plot_P_T_perturbations(model, scale, var, timeframe, mode, diftype, plotsave, y0, ye, scenario):
    ...
    base_folder_in = f"/Users/magaliponds/OneDrive - Vrije Universiteit Brussel/1. VUB/02. Research/01. IRRMIP/03. Data/03. Output files/01. Climate data/01. Processed input data/{var}/{timeframe}/{model}/{member}/SSP{scenario}/"
    diff_folder_in = f"/Users/magaliponds/OneDrive - Vrije Universiteit Brussel/1. VUB/02. Research/01. IRRMIP/03. Data/03. Output files/01. Climate data/02. Perturbations/{var}/{timeframe}/{model}/{member}/SSP{scenario}/"

    ifile_diff = f"{diff_folder_in}/{model}.{var_suffix}.SSP{scenario}.DIF.00{member}.{y0}_{ye}_{timeframe}_{diftype}.nc"
    ifile_irr = f"{base_folder_in}/{model}.{var_suffix}.SSP{scenario}.IRR.00{member}.{y0}_{ye}_{timeframe}_{diftype}.nc"
    ifile_noi = f"{base_folder_in}/{model}.{var_suffix}.SSP{scenario}.NOI.00{member}.{y0}_{ye}_{timeframe}_{diftype}.nc"
    ...
    o_folder_base = f"/Users/magaliponds/OneDrive - Vrije Universiteit Brussel/1. VUB/02. Research/01. IRRMIP/04. Figures/01. Climate data/01. Input data/{scale}/{var}/{model}/{member}/SSP{scenario}"
    o_folder_diff = f"/Users/magaliponds/OneDrive - Vrije Universiteit Brussel/1. VUB/02. Research/01. IRRMIP/04. Figures/01. Climate data/02. Perturbations/{scale}/{var}/{model}/{member}/SSP{scenario}"
    ...
    if plotsave == 'save':
        ...
        o_file_name = f"{o_folder_diff}/{model}.{var_suffix}.SSP{scenario}.DIF.00{member}.{y0}_{ye}_{timeframe}_{diftype}.png"
        plt.savefig(o_file_name, bbox_inches='tight')
    plt.show()
    return


# %% Cell 3: Run the perturbation processing for all climate models, members etc.
...
for (m, model) in enumerate(["CESM2"]):
    for member in range(members[m]):
        if member != 0:
            for s, scenario in enumerate(["126", "370"]):
                    ...
                    for timeframe in ["annual", "seasonal", "monthly"]:
                        for mode in ['dif']:  # , 'std']:
                            if var == "Precipitation" and mode == 'dif':
                                ...
                                diftypes = ['abs']
                            for dif in diftypes:
                                print(var, model, member,
                                      timeframe, dif, scenario)

                                process_P_T_perturbations(
                                    model, member, var, timeframe, mode, dif, y0, ye, scenario)


# %% Cell 4: Run the plotting functions for all different combinations to generate output datasets and plots
members = [4]  # 1, 3, 4, 6, 3]
y0 = 2015  # 1901
ye = 2074  # 1930
                                ...
                                for dif in diftypes:
                                    for scenario in ["126", "370"]:
                                        ...
                                        plot_P_T_perturbations(
                                            model, scale, var, timeframe, mode, dif, "save", y0, ye, scenario)
