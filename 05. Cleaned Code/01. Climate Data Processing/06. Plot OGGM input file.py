...
""" This file plots the created perturbed climate files for  a quick scan """

""" It does so in the following steps:
    1. Plotting fucntion for perturbed climate input files as created in step 5
    2. Looping through the plot function for all different modelxmember combinations
    
    """



#%% Cell 0: Load directories
import xarray as xr
...
# %% Cell 1: Plot climates incl. perturbation


def plot_P_T_perturbed_climate_files(model, scale, var, timeframe, plotsave, y0, ye):

    if ye == 1985:
        cf_suffix = ".counterfactual"
        cf_title = "No Forcings"
    else:
        cf_suffix = ""
        cf_title = "No Irrigation"
    ...
    i_folder_clim = f"/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/03. Data/03. Output files/01. Climate data/06. OGGM Climate input files/{timeframe}/W5E5/0"
    ifile_base = f"{i_folder_clim}/W5E5.000.1985_2014.{timeframe}.perturbed.climate.input.degC.nc"
    base_data = xr.open_dataset(ifile_base)[var_idx]
    if model == "W5E5":
        data = base_data
    else:
        i_folder_delta = f"/Users/magaliponds/Library/CloudStorage/OneDrive-VrijeUniversiteitBrussel/1. VUB/02. Coding/01. IRRMIP/03. Data/03. Output files/01. Climate data/07. OGGM Perturbation input files/{timeframe}/{model}/{member}"
        ifile_delta = f"{i_folder_delta}/{model}.00{member}.{y0}_{ye}.{timeframe}.perturbation.input.%.degC{cf_suffix}.nc"
        delta = xr.open_dataset(ifile_delta)[var_idx]
        # Add perturbations
        if var == "Precipitation":
            # Relative perturbation for precipitation
            data = base_data - (base_data*delta)
        if var == "Temperature":
            data = base_data - delta  # Absolute perturbation for temperature

    if scale == "Local":
        data = data.where((data.lon >= 60) & (data.lon <= 109) & (
            data.lat >= 22) & (data.lat <= 52), drop=True)
    ...
    for time_idx, timestamp_name in enumerate(timestamps):
        ...
        if isinstance(data_sel, xr.Dataset):
            data_sel = data_sel[list(
                data_sel.data_vars.keys())[0]]

        # plot the data incl the outline of the karakoram shapefile, setting the colors, but excluding the shapefile
        im = data_sel.plot.imshow(ax=ax, vmin=vmin, vmax=vmax, extend='both', transform=ccrs.PlateCarree(
        ), cmap=custom_cmap, add_colorbar=False)
        ...
        min_value_index = np.unravel_index(
            np.argmin(data_sel_min.values), data_sel_min.shape)
        max_value_index = np.unravel_index(
            np.argmax(data_sel_max.values), data_sel_max.shape)

        # Extract longitude and latitude corresponding to the minimum and maximum value indices
        min_lon, min_lat = data_sel.lon.values[min_value_index[1]
                                               ], data_sel.lat.values[min_value_index[0]]
        max_lon, max_lat = data_sel.lon.values[max_value_index[1]
                                               ], data_sel.lat.values[max_value_index[0]]

        # Plot the dot on the subplot
        # Adjust marker properties as needed
        ...
        if scale == "Local":
            if timeframe == 'annual':
                ...
                ax.plot(62, 49, marker='o',
                        color='blue', markersize=5)
                ax.plot(82, 49, marker='o',
                        color='red', markersize=5)
        if scale == "Global":
            if timeframe == 'monthly':
    ...
    if timeframe == 'monthly':
        fig.text(0.5, 0.03, 'Longitude',
                 ha='center', fontsize=15)
        fig.text(0.03, 0.5, 'Latitude', va='center',
                 rotation='vertical', fontsize=15)
    else:
        fig.text(0.5, 0.01, 'Longitude',
                 ha='center', fontsize=15)
        fig.text(-0.02, 0.5, 'Latitude', va='center',
                 rotation='vertical', fontsize=15)

    if model == "W5E5":
        fig.text(0.5, 0.92, str(model) + " All Forcings",
                 ha='center', fontsize=15)
    else:
        fig.text(0.5, 0.92, str(model) + " 00" + str(member) +
                 f" {cf_title}", ha='center', fontsize=15)
    ...
    if plotsave == 'save':
        ...
        o_file_name = f"{o_folder_data}/{model}.00{member}.1985_2014.{timeframe}.perturbed.climate.input{cf_suffix}.png"
...
# %% Cell 2: Loop through the different plotting functions for all different model x member combinations
y0s = [1985, 1901]
yes = [2014, 1985]
members = [1, 3, 4, 6, 1, 4]  # 1
# "E3SM","CESM2","CNRM","IPSL-CM6"]):#"W5E5",
for (m, model) in enumerate(["W5E5", "E3SM", "CESM2", "CNRM", "IPSL-CM6", "NorESM"]):
            ...
            for var in ["Temperature", "Precipitation"]:
                for timeframe in ["annual"]:  # , "seasonal", "monthly"]:
                    for y, y0 in enumerate(y0s):
                        ...
                        plot_P_T_perturbed_climate_files(
                            model, scale, var, timeframe, "save", y0, ye)
